...
public:
	...
	~VulkanExample()
	{
		...
		vkDestroyPipeline(device, pipelines.debugDisplay, nullptr);
		...
		vkDestroyPipelineLayout(device, pipelineLayouts.debugDisplay, nullptr);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			if (displayShadowMap) {
				vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.debugDisplay, 0, 1, &cascades[displayCascadeIndex].descriptorSet, 0, NULL);
				vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.debugDisplay);
				vkCmdDraw(drawCmdBuffers[i], 3, 1, 0, 0);
			}

			// Render shadowed scene
			vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.debugDisplay, 0, 1, &descriptorSet, 0, NULL);
	...
	void setupDescriptorSetLayout()
	{
		...
		VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayouts.debugDisplay));
	...
	void preparePipelines()
	{
		...
		VkGraphicsPipelineCreateInfo pipelineCreateInfo =
			vks::initializers::pipelineCreateInfo(pipelineLayouts.debugDisplay, renderPass, 0);
		...
		shaderStages[0] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/debugdisplay.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/debugdisplay.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		// Empty vertex input state
		VkPipelineVertexInputStateCreateInfo emptyInputState = vks::initializers::pipelineVertexInputStateCreateInfo();
		pipelineCreateInfo.pVertexInputState = &emptyInputState;
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.debugDisplay));
	...
	virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay)
	{
		if (overlay->header("Settings")) {
			...
			if (overlay->checkBox("Display shadow cascade", &displayShadowMap)) {
				buildCommandBuffers();
			}
			if (displayShadowMap) {
				if (overlay->sliderInt("Part to render", &displayCascadeIndex, 0, SHADOW_MAP_CASCADE_COUNT - 1)) {
					buildCommandBuffers();
				}
			}
